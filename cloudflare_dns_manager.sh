...
# Function to add segmented TXT record
add_segmented_txt_record() {
    local name=$1
    local ttl=$2
    shift 2
    local contents=("$@")
    ...
    for content in "${contents[@]}"; do
        # Extract the segment number and content
        segment_num=$(echo "$content" | cut -d':' -f1)
        segment_content=$(echo "$content" | cut -d':' -f2-)
        
        # Escape double quotes in the content
        segment_content=$(echo "$segment_content" | sed 's/"/\\"/g')
        
        echo "Adding TXT record segment $segment_num"
        add_dns_record "TXT" "${name}" "\"${segment_num}:${segment_content}\"" "$ttl"
...
case "$1" in
  "add")
    if [ "$2" = "TXT" ] && [[ "$4" == *":"* ]]; then
      # For segmented TXT records
      add_segmented_txt_record "$3" "${@: -1}" "${@:4:$#-4}"
    else
  ...
  *)
    ...
    echo "  $0 add TXT example.com '1:content part 1' '2:content part 2' '3:content part 3' 3600"
    echo "  $0 get_all"
    echo "  $0 get A"
    exit 1
    ;;
